...
from utils.utils_math import TSVector3,BaseTSVector3
import numpy as np
import copy, math
import random
...
class FDU2021_agent(Agent):
    ...
    def _init(self):
        ...
        self.danger_enemy = defaultdict(list)   # 该变量用于记录所有对我方态势占优的敌机
        self.my_attacking_enemy = {}  # 该变量用于记录正在去攻击的飞机及其目标
        ...
        self.own_features = {'x': 0, 'y': 0, 'z': 0, 'head': 0, 'pitch': 0, 'roll': 0, 'speed': 0, 'islocked': 0,
                             'leftweapon': 0}
    ...
    def process_decision(self, sim_time, obs_side, cmd_list):
        ...
        if sim_time == 2:  # 当作战时间为2s时,开始进行任务开始,并保存任务指令
            # self.mission_test(sim_time, cmd_list)
            # print(self.cal_rel_state(self.my_manned_info[0],self.my_uvas_infos[0]))
            self.mission_start(sim_time, cmd_list)  # 调用任务判断以及处理，进入决策阶段
        #
        if sim_time > 250:  # 当作战时间大于10s时,开始进行任务控制,并保存任务指令
            for plane_info in self.my_allplane_infos:  # 遍历所有的己方无人飞机
                if self.get_available_action(plane_info)[10]:
    ...
    def process_observation(self, obs_side):
        ...
        for uvas_info in my_entity_infos:
            if uvas_info['ID'] != 0 and uvas_info[
                'Availability'] > 0.0001:  # 判断飞机是否可用 飞机的ID即为飞机的唯一编号 飞机的Availability为飞机当前生命值
        ...
        for missile_info in missile_infos:
            if missile_info['LauncherID'] in my_allplane_maps:  # 判断导弹是否为己方导弹 导弹的LauncherID即为导弹的发射者
                ...
                continue
            if (missile_info['ID'] != 0 and missile_info[
                'Availability'] > 0.0001):  # 判断导弹是否可用 导弹的ID即为导弹的唯一编号 导弹的Availability为导弹当前生命值
        ...
        for my_id, enemy_id in list(self.my_attacking_enemy.items()):
            if not self.enemy_is_alive(enemy_id):
                ...
                enemy_info = self.get_enemy_by_id(enemy_id)
                my_info = self.get_unit_by_id(my_id)
                coord_a = [my_info['X'],my_info['Y'],my_info['Alt']]
                coord_b = [enemy_info['X'], enemy_info['Y'], enemy_info['Alt']]
                dis = self.cal_distance(coord_a,coord_b)
                if dis < 20000:
                    self.my_attacking_enemy.pop(my_id)
    ...
    def get_danger_enemys(self):
        ...
        for plane_info in self.my_allplane_infos:
            ...
            for enemy in self.enemy_allplane_infos:
                [qr, qb, dis, belta, rel_h, rel_speed, speed_r, z_r] = self.cal_rel_state(plane_info, enemy)
                if plane_info['Type'] == 1:  # 有人机
                    if dis < 80000 and qr > np.pi / 6 and qb < np.pi / 6:  # 有人机劣势
                        self.danger_enemy[plane_info['ID']].append(enemy)
                else:  # 我方是无人机
                    if enemy['Type'] == 2 and dis < 80000 and qb < np.pi / 6:  # 劣势
                        self.danger_enemy[plane_info['ID']].append(enemy)
                    elif enemy['Type'] == 1 and dis < 80000 and qr > np.pi / 3 and qb < np.pi / 6:
                        self.danger_enemy[plane_info['ID']].append(enemy)
                    else:
                        pass
            enemy_missiles = self.get_enemy_missile_by_id(plane_info['ID'])
            for missile in enemy_missiles:
                dis = self.cal_distance([missile['X'], missile['Y'], missile['Alt']], [x, y, z])
                enemy_id = missile['LauncherID']
    ...
    #             # 计算两个飞机之前的方位角与偏航角差
    #             [dis, rel_heading, rel_pitch] = self.cal_rel_eular_angle(my_plane_info, enemy_plane)
    #             if dis < data['launch_range'] and np.abs(rel_heading) < data['max_heading_range'] and np.abs(
    #                     rel_pitch) < data['max_pitch_range']:
    ...
    def init_pos(self, cmd_list):
        ...
        for leader in self.my_manned_info:  # 为己方有人机设置初始位置
            ...
            cmd_list.append(CmdEnv.make_entityinitinfo(leader['ID'], leader_original_pos['X'], leader_original_pos['Y'],
                                                       leader_original_pos['Z'], 400 * 0.8, 45))
        ...
        for sub in self.my_uvas_infos:  # 为己方每个无人机设置初始位置
            ...
            if sub_index & 1 == 0:  # 将当前编号放在有人机的一侧
                # CmdEnv.make_entityinitinfo 指令，可以将 飞机实体置于指定位置点，参数依次为 实体ID，X坐标，Y坐标，Z坐标，初始速度，初始朝向
                cmd_list.append(
                    CmdEnv.make_entityinitinfo(sub['ID'], sub_pos['X'], sub_pos['Y'] + interval_distance, sub_pos['Z'],
                                               300 * 0.9, 45))
            else:  # 将当前编号放在有人机的另一侧
                # CmdEnv.make_entityinitinfo 指令，可以将 飞机实体置于指定位置点，参数依次为 实体ID，X坐标，Y坐标，Z坐标，初始速度，初始朝向
                cmd_list.append(
            ...
            if plane['ID'] not in self.attack_handle_enemy:  # 如果当前飞机机不处于等待攻击状态
                ...
                if len(self.enemy_manned_info) > 0:
                    ...
                    if sim_time % 10 == 0:
                        ...
                        cmd_list.append(
                            CmdEnv.make_motioncmdparam(plane['ID'], 1, speed, data['move_max_acc'], data['move_max_g']))
                        ...
                        cmd_list.append(CmdEnv.make_followparam(plane['ID'], enemy_leader['ID'], data['move_max_speed'],
                    ...
                    cmd_list.append(
                        CmdEnv.make_areapatrolparam(plane['ID'], plane['X'], plane['Y'], data['area_max_alt'], 200, 100,
    ...
    def select_target(self, own_plane, enemy_list) -> {}:
        ...
        :return:最近的敌方飞机信息
        """
        min_distance = 80000  # 超过此距离的敌方飞机不在考虑之内
        close_enemy = {}
    ...
    def cal_rel_state(self, unit_agent, unit_enemy):
        ...
        qr = np.arccos(((x_b - x_r) * np.cos(chi_r) * np.cos(tau_r) + (y_b - y_r) * np.sin(chi_r) * np.cos(tau_r) + (
                    z_b - z_r) * np.sin(tau_r)) / dis)
        qb = np.arccos(((x_r - x_b) * np.cos(chi_b) * np.cos(tau_b) + (y_r - y_b) * np.sin(chi_b) * np.cos(tau_b) + (
                    z_r - z_b) * np.sin(tau_b)) / dis)
        belta = np.arccos(
            np.cos(chi_r) * np.cos(tau_r) * np.cos(chi_b) * np.cos(tau_b) + np.cos(tau_r) * np.sin(tau_r) * np.cos(
                tau_b) * np.sin(tau_b) + np.sin(tau_r) * np.sin(tau_b))
        rel_h = z_r - z_b
        rel_speed = speed_r - speed_b
        return [qr, qb, dis, belta, rel_h, rel_speed, speed_r, z_r]

    def cal_rel_eular_angle(self, unit_agent, unit_enemy):
        x_g = unit_enemy['X'] - unit_agent['X']
        y_g = unit_enemy['Y'] - unit_agent['Y']
        z_g = unit_enemy['Alt'] - unit_agent['Alt']
        dis = math.sqrt(x_g * x_g + y_g * y_g + z_g * z_g)
        coord_g = np.array([[x_g], [y_g], [z_g]])

        yaw = unit_agent['Heading']
        pitch = unit_agent['Pitch']
        roll = unit_agent['Roll']

        r_z = np.array([[np.cos(yaw), np.sin(yaw), 0], [-np.sin(yaw), np.cos(yaw), 0], [0, 0, 1]])
        r_x = np.array([[1, 0, 0], [0, np.cos(pitch), np.sin(pitch)], [0, -np.sin(pitch), np.cos(pitch)]])
        r_y = np.array([[np.cos(roll), 0, -np.sin(roll)], [0, 1, 0], [np.sin(roll), 0, np.cos(roll)]])

        temp1 = np.dot(r_z, coord_g)
        temp2 = np.dot(r_x, temp1)
        temp3 = np.dot(r_y, temp2)

        coord_b = {'X': temp3[0], 'Y': temp3[1], 'Z': temp3[2]}
        return [dis, self.cal_heading(coord_b), TSVector3.calpitch(coord_b)]
        ...
        if unit and unit['Availability'] > 0.01:  # 当前飞机还活着
            ...
            for i in range(self.n_enemies):
                ...
                if enemy_unit and enemy_unit['Availability'] > 0.01:  # 这个飞机还存在
                    ...
                    enemy_feats[i][2] = (unit['Alt'] - enemy_unit['Alt']) / (
                                own_move_data['area_max_alt'] - enemy_move_data['area_min_alt'])
                    enemy_feats[i][3] = enemy_unit['Heading'] / np.pi
                    enemy_feats[i][4] = enemy_unit['Pitch'] / np.pi
                    enemy_feats[i][5] = (enemy_unit['Speed'] - enemy_move_data['move_min_speed']) / (
                                enemy_move_data['move_max_speed'] - enemy_move_data['move_min_speed'])
            ...
            for i in range(self.n_agents):
                ...
                if ally_unit and ally_unit['Availability'] > 0.01 and ally_id != agent_id:
                    ...
                    ally_feats[inc][2] = (unit['Alt'] - ally_unit['Alt']) / (
                                own_move_data['area_max_alt'] - ally_move_data['area_min_alt'])
                    ally_feats[inc][3] = ally_unit['Heading'] / np.pi
                    ally_feats[inc][4] = ally_unit['Pitch'] / np.pi
                    ally_feats[inc][5] = (ally_unit['Speed'] - ally_move_data['move_min_speed']) / (
                                ally_move_data['move_max_speed'] - ally_move_data['move_min_speed'])
                    ally_feats[inc][6] = ally_unit['IsLocked']
                    inc += 1

            own_feats[0][0] = 2 * unit['X'] / self.map_len
            own_feats[0][1] = 2 * unit['Y'] / self.map_width
            own_feats[0][2] = (unit['Alt'] - own_move_data['area_min_alt']) / (
                        own_move_data['area_max_alt'] - own_move_data['area_min_alt'])
            own_feats[0][3] = unit['Heading'] / np.pi
            own_feats[0][4] = unit['Pitch'] / np.pi
            own_feats[0][5] = unit['Roll'] / np.pi
            own_feats[0][6] = (unit['Speed'] - own_move_data['move_min_speed']) / (
                        own_move_data['move_max_speed'] - own_move_data['move_min_speed'])
            ...
            for i in range(obs_own_missile_nums):
                if i < len(own_missiles) and own_missiles[i]['Availability'] > 0.001:
                    ...
                    own_missile_feats[i][2] = (unit['Alt'] - missile_unit['Alt']) / (
                                own_move_data['area_max_alt'] - missile_move_data['area_min_alt'])
                    own_missile_feats[i][3] = missile_unit['Heading'] / np.pi
                    own_missile_feats[i][4] = missile_unit['Pitch'] / np.pi
                    own_missile_feats[i][5] = (missile_unit['Speed'] - missile_move_data['move_min_speed']) / (
                                missile_move_data['move_max_speed'] - missile_move_data['move_min_speed'])
            ...
            for i in range(obs_enemy_missile_nums):
                if i < len(enemy_missiles) and enemy_missiles[i]['Availability'] > 0.001:
                    ...
                    enemy_missile_feats[i][2] = (unit['Alt'] - missile_unit['Alt']) / (
                                own_move_data['area_max_alt'] - missile_move_data['area_min_alt'])
                    enemy_missile_feats[i][3] = missile_unit['Heading'] / np.pi
                    enemy_missile_feats[i][4] = missile_unit['Pitch'] / np.pi
                    enemy_missile_feats[i][5] = (missile_unit['Speed'] - missile_move_data['move_min_speed']) / (
                                missile_move_data['move_max_speed'] - missile_move_data['move_min_speed'])
            ...
            return np.zeros(self.get_obs_size())
    ...
    def get_rotation_matrix_gb(self, theta, psi, phi):
        matri = np.array([[np.cos(theta) * np.cos(psi),
    ...
    def select_nearest_target(self, own_plane, enemy_list):
        ...
        for enemy in enemy_list:
            ...
            if enemy['Type'] == 1 and distance < min_distance:  # 除非是有人挤才允许同时被多个我方飞机在攻击
                min_distance = distance
                close_enemy = enemy
            elif own_plane['ID'] not in self.my_attacking_enemy and enemy['ID'] not in self.my_attacking_enemy.values() and distance < min_distance:  # 计算出一个离得最近的敌方实体
    ...
    def get_action(self, action_num, plane_info, cmd_list):
        data = self.get_move_data(plane_info)  # 根据拿到飞机获取对应的机动数据
        x, y, z = [plane_info['X'], plane_info['Y'], plane_info['Alt']]
        z = np.clip(z, data['area_min_alt'], data['area_max_alt'])
        if action_num == 0:  # 定常飞行
            ...
            # CmdEnv.make_motioncmdparam 指令，可以将修改当前飞机的速度和超载，参数依次为 飞机ID，调整机动参数（1为调整速度，2调整加速度，3调整速度和加速度） ，速度，加速度，超载（与飞机转向角度有关）
            cmd_list.append(
                CmdEnv.make_motioncmdparam(plane_info['ID'], 1, speed, data['move_max_acc'], data['move_max_g']))
            ...
            if len(self.enemy_manned_info) > 0:
                enemy_leader = self.enemy_manned_info[0]  # 拿到敌方有人机信息
                leader_fire_route_list = [{"X": enemy_leader['X'], "Y": enemy_leader['Y'],
                                           "Z": data['area_max_alt']}, ]  # 获取一个路径点,将敌方有人机的所在点设置为目标点
                # EnvCmd.make_linepatrolparam 指令，可以令飞机沿给定路线机动，参数依次为 飞机ID，路线，速度，加速度，超载（与飞机转向角度有关）
                cmd_list.append(
                    CmdEnv.make_linepatrolparam(plane_info['ID'], leader_fire_route_list, data['move_max_speed'],
                                                data['move_max_acc'], data["move_max_g"]))
            else:  # 如果敌有人机阵亡
                # EnvCmd.make_areapatrolparam 指令，可以令飞机绕给定点巡逻，参数依次为 飞机ID，给定点X坐标，给定点Y坐标，给定点Z坐标，给定区域的长度，给定区域的宽度，速度，加速度，超载（与飞机转向角度有关）
                cmd_list.append(
                    CmdEnv.make_areapatrolparam(plane_info['ID'], plane_info['X'], plane_info['Y'],
                                                data['area_max_alt'], 200, 100,
                                                data['move_max_speed'], data['move_max_acc'], data['move_max_g']))
            ...
            if plane_info['ID'] in self.my_attacking_enemy:
                ...
                Alt = np.clip(attack_info['Alt'], data['area_min_alt'], data['area_max_alt'])
                target_fire_route_list = [{"X": attack_info['X'], "Y": attack_info['Y'], "Z": Alt}, ]
                cmd_list.append(
        ...
        elif action_num == 10:  # 主动规避,绕这个enemy旋转90度，并且高度降到最低高度上方200m
            enemy_list = self.danger_enemy[plane_info['ID']]
            close_enemy = self.select_target(plane_info, enemy_list)['enemy_data']
            e_x, e_y, e_z = [close_enemy['X'], close_enemy['Y'], close_enemy['Alt']]

            # 计算以enemy为中心顺时针旋转angle角度后的位置，需在此判断方向
            close_enemy['Z'] = close_enemy['Alt']
            plane_info['Z'] = plane_info['Alt']
            rel_vec = BaseTSVector3.minus(close_enemy, plane_info)
            direction = BaseTSVector3.cross(
                {"X": plane_info['Speed'] * np.cos(plane_info['Pitch']) * np.sin(plane_info['Heading']), \
            ...
            if BaseTSVector3.dot(direction, {"X": 0, "Y": 0, "Z": 1}) > 0:
                angle = np.pi / 2  # 绕敌机逆时针旋转
            else:
                angle = -np.pi / 2  # 顺时针
            # angle = np.pi / 2  # 顺时针
            tar_x = (x - e_x) * np.cos(angle) - (y - e_y) * np.sin(angle) + e_x
            tar_y = (x - e_x) * np.sin(angle) + (y - e_y) * np.cos(angle) + e_y
            tar_x = np.clip(tar_x, -self.map_len / 2, self.map_len / 2)
            tar_y = np.clip(tar_y, -self.map_width / 2, self.map_width / 2)
            tar_z = data['area_min_alt']
            curr_pos = [{"X": tar_x, "Y": tar_y, "Z": tar_z}, ]
            cmd_list.append(
                CmdEnv.make_linepatrolparam(plane_info['ID'], curr_pos, data['move_max_speed'],
                                            data['move_max_acc'], data["move_max_g"]))

        elif action_num == 11:  # 最后一个动作表示的是只有已经死亡的飞机才可以执行的动作
            pass
        else:
            pass
        return cmd_list
    ...
    def get_available_action(self, plane_info):
        ...
        # 判断有可以攻击的对象
        left_ok = False
        if plane_info['Type'] == 1 and plane_info['LeftWeapon'] > 1:
            left_ok = True
        elif plane_info['Type'] == 2 and plane_info['LeftWeapon'] > 0:
            left_ok = True
        else:
            left_ok = False
        if plane_info['ID'] in self.in_range_enemy and left_ok:
            enemy_list = self.in_range_enemy[plane_info['ID']]
            close_enemy = self.select_nearest_target(plane_info, enemy_list)
            if close_enemy:
                available_action[9] = 1
        ## 判断当前飞机是否需要执行躲避机动
        if plane_info['ID'] in self.danger_enemy:
            if plane_info['Type'] == 1:
                available_action[10] = 1
            elif plane_info['ID'] not in self.my_attacking_enemy:
                available_action[10] = 1
