...
class FDU2021_agent(Agent):
    ...
    def _init(self):
        ...
        self.danger_enemy = defaultdict(list)
    ...
    def init_pos(self, cmd_list):
        ...
        for leader in self.my_manned_info:  # 为己方有人机设置初始位置
            ...
            cmd_list.append(CmdEnv.make_entityinitinfo(leader['ID'], leader_original_pos['X'], leader_original_pos['Y'],
                                                       leader_original_pos['Z'], 400 * 0.6, 45))
    ...
    def get_action(self, action_num, plane_info, cmd_list):
        ...
        if action_num == 0:  # 定常飞行
            ...
            rel_vec = [BaseTSVector3.minus(close_enemy, plane_info)['X'], BaseTSVector3.minus(close_enemy, plane_info)['Y']]
            speed_vec = [np.sin(plane_info['Heading']), np.cos(plane_info['Heading'])]
            if (speed_vec[0]*rel_vec[1] - speed_vec[1]*rel_vec[0]) > 0:
                angle = -np.pi / 2
            else:
                angle = np.pi / 2
